...
namespace Au.More {
    ...
    /// </summary>
    /// <remarks>
    /// If server is global (see constructor), clients can be multiple processes, including this, but there is only a single active global server.  If local then it is only the print server for this process.
    ...
    /// <code><![CDATA[
    /// /*/ role exeProgram; outputPath %folders.Workspace%\exe\OutputFormExample; /*/
    ...
    /// 		_printServer.SetNotifications(this.Hwnd(), WM_APP);
    ...
    /// 	{
    /// 		while(_printServer.GetMessage(out var m)) {
    /// 			switch(m.Type) {
    ...
    /// 	}
    /// 
	///		// if try isGlobal: true, can get output here but need to close LA, otherwise output goes there
    /// 	static PrintServer _printServer = new(isGlobal: false);
    /// 
    /// 	[STAThread]
    /// 	static void Main()
    /// 	{
    ...
    /// 		Task.Run(() => { 2.s(); print.it("test after"); 2.s(); print.clear(); 2.s(); print.it("test after Clear"); });
    /// 
    ...
    public unsafe class PrintServer {
		...
		/// <param name="isGlobal">
		/// If <c>true</c>, will receive output from all processes that don't have local server.  Only one global server receives messages.  By default this is the output panel of the code editor if it is running.
		/// </param>
